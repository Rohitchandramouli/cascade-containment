...
| --- | --- | --- |
| Infection penalty | `−0.50 × density` | District infection > 0.40 |
| Hospital breach | `−1.00` | Hospital capacity ≤ 10% |
...
Fully deterministic — no randomness, no LLM calls. Identical trajectories always produce identical scores in `[0.0, 1.0]`.
...
│
├── scripts/
│   └── test_local.py         # Phase 1 spec checks + greedy benchmark + variance analysis
│
└── core/
    ├── trajectory.py         # EpisodicMemory — L1 similarity retrieval, phase-weighted
...
| Phase 1 automated validation | ✅ 10/10 checks pass |
